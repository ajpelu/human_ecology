...
```{r model-rec_total}
bm <- lmer(
  fitovol_dm3 ~ sr.spr + sr.sum + ph + potasio + commons + transh + commons:transh + (1 | parcela), 
  data = d
)
...
```{r model-fitovol}
bm <- lmer(
  fitovol_dm3 ~ sr.sum + sr.spr + commons + transh + commons:transh + (1 | parcela), 
  data = d
)
...
```{r model-tasacons}
bm <- lmer(
  tasa_cons ~ 1 + sr.sum + cic + commons + transh + commons:transh + (1 | parcela), data = d
)
